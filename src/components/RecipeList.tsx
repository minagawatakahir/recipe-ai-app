...
export const RecipeList: React.FC<RecipeListProps> = ({ 
  ...
  const handleSearch = (keyword: string) => {
    ...
    if (keyword.trim()) {
      setFilteredRecipes(onSearch(keyword));
    } else {
      setFilteredRecipes(recipes);
    }
  };

  const displayRecipes = searchKeyword.trim() ? filteredRecipes : recipes;
    ...
    <div className="bg-white shadow-md rounded-lg p-6">
      ...
      {displayRecipes.length === 0 ? (
        <p className="text-gray-500 text-center py-8">
          {searchKeyword ? '検索結果がありません' : '保存されたレシピはまだありません'}
        </p>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {displayRecipes.map((recipe) => (
            <div
              key={recipe.id}
              className="border border-gray-200 rounded-lg p-4 hover:shadow-lg transition duration-200 cursor-pointer"
